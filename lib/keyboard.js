 ...
 * Dependencies.
 */
// var debug = require('debug')('noVNC:Keyboard');
var debugerror = require('debug')('noVNC:ERROR:Keyboard');
...
var Keyboard = module.exports = {
	...
	hasShortcutModifier: function (charModifier, currentModifiers) {
		...
		if (Keyboard.hasCharModifier(charModifier, mods)) {
	...
	ModifierSync: function (charModifier) {
		...
		function syncKeyEvent(evt, down) {
			var obj = Keyboard.getKeysym(evt);
		...
		return {
			...
			hasShortcutModifier: function() {
				return Keyboard.hasShortcutModifier(charModifier, state);
			},
			// if a char modifier is down, return the keys it consists of, otherwise return null
			activeCharModifier: function() {
				return Keyboard.hasCharModifier(charModifier, state) ? charModifier : null;
			}
		};
	},
	...
	getKeysym: function (evt) {
		...
		if (codepoint) {
			var res = Keys.fromUnicode(Keyboard.substituteCodepoint(codepoint));
			if (res) {
				return res;
			}
		}
		...
		if (evt.keyCode) {
			return Keys.lookup(Keyboard.keysymFromKeyCode(evt.keyCode, evt.shiftKey));
		}
		if (evt.which) {
			return Keys.lookup(Keyboard.keysymFromKeyCode(evt.which, evt.shiftKey));
		}
		return null;
	},
	...
	keysymFromKeyCode: function (keycode, shiftPressed) {
		...
		return Keyboard.nonCharacterKey({keyCode: keycode});
	},
	...
	KeyEventDecoder: function (modifierState, next) {
		...
		function process(evt, type) {
			...
			var keyId = Keyboard.getKey(evt);

			if (keyId) {
				result.keyId = keyId;
			}

			var keysym = Keyboard.getKeysym(evt);
			...
			if (keysym && (type !== 'keydown' || Keyboard.nonCharacterKey(evt) || hasModifier)) {
			...
			var suppress = !isShift && (type !== 'keydown' || modifierState.hasShortcutModifier() || !!Keyboard.nonCharacterKey(evt));

			// If a char modifier is down on a keydown, we need to insert a stall,
			// so VerifyCharModifier knows to wait and see if a keypress is comnig
			var stall = type === 'keydown' && modifierState.activeCharModifier() && !Keyboard.nonCharacterKey(evt);
